 ...
 *
 */
export const useNuiRequest = ({ resource } = {}) => {
    const context = useContext(NuiContext);
    if (!context) {
        throw new Error("react-fivem-nui-utils: useNuiRequest must be used inside NuiProvider passing the `resource` prop");
    }
    const { send, sendAbortable } = context;
    return useMemo(() => ({
        send: (event, data = {}) => send(event, data, resource),
        sendAbortable: (event, data = {}) => sendAbortable(event, data, resource),
